  ...
  <div id="sort">
    <{foreach from=$faq item=faq}>
      <{if $faq.enable=="1" or $isAdmin or $edit_power}>
        ...
        <div id="tr_<{$faq.fqsn}>_ans" class="well faq_content" style="line-height: 1.8;">
          <{if $isAdmin or $edit_power}>
            <div style="text-align:right;">
              <a href="index.php?op=update_status&fcsn=<{$fcsn}>&fqsn=<{$faq.fqsn}>&enable=<{$faq.update_enable}>" class="btn btn-xs btn-info"><{$faq.enable_txt}></a>

              <a href="javascript:delete_tad_faq_content_func(<{$faq.fqsn}>);" class="btn btn-xs btn-danger"><{$smarty.const._TAD_DEL}></a>

              <a href="index.php?op=tad_faq_content_form&fqsn=<{$faq.fqsn}>" class="btn btn-xs btn-warning"><{$smarty.const._TAD_EDIT}></a>
            </div>
          <{/if}>
          <{$faq.content}>
        </div>
